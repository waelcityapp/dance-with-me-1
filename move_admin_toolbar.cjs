...
// 1. Remove it from its current location inside the media wrapper
const idx = content.indexOf(adminToolbarBlock);
if (idx !== -1) {
  content = content.substring(0, idx) + content.substring(idx + adminToolbarBlock.length);
}

// 2. Insert it before the media wrapper starts
...
console.log('done moving admin toolbar to root of EventCard');
