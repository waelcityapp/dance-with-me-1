...
import { DanceEvent, UserProfile, NotificationItem, AdSubmission, AdChangeRequest, SupportMessage, EventBooking, AccountTier } from '../types';
 ...
 */
export async function resetFirebasePassword(email: string, lang: 'ar' | 'en' = 'ar'): Promise<boolean> {
  try {
 ...
 */
export async function checkAndSeedEvents(_initialEvents: DanceEvent[]): Promise<void> {
  // Administration unlock codes are deliberately stored only in Vercel environment variables.
}
...
export function subscribeToAdChangeRequests(onUpdate: (items: AdChangeRequest[]) => void): () => void {
  try {
    return onSnapshot(collection(db, 'ad_change_requests'), (snapshot) => {
      const items = snapshot.docs.map(item => ({ id: item.id, ...item.data() } as AdChangeRequest));
      items.sort((a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime());
      onUpdate(items);
    }, (error) => console.warn('Firestore ad change requests subscribe error:', error));
  } catch (error) {
    console.warn('Failed to setup ad change requests subscription:', error);
    return () => {};
  }
}
