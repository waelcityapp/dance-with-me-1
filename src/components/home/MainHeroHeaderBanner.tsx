      ...
      {activeCategoryId && !isCategoryMenuOpen && desktopMenuPosition && createPortal(
        ...
        >
          <div className="mb-2 flex shrink-0 items-center justify-between gap-3 border-b border-[#d4af67]/30 px-1 pb-2">
            <button type="button" onClick={() => setActiveCategoryId(null)} aria-label={isAr ? 'إغلاق التصنيفات' : 'Close subcategories'} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[#e8c978] transition hover:bg-[#791524] hover:text-white">
              <X className="h-4 w-4" />
            </button>
            <div className="min-w-0 text-end">
              <h3 className="truncate text-sm font-black text-white">{isAr ? 'تصنيفات' : 'Categories'} {isAr ? activeCategory?.ar : activeCategory?.en}</h3>
              <p className="mt-0.5 text-[10px] text-[#e7c98b]/80">{isAr ? 'اختر تصنيفًا لعرض الإعلانات المناسبة' : 'Choose a category to filter listings'}</p>
            </div>
          </div>
          <div className="grid min-h-0 grid-cols-1 gap-1.5 overflow-y-auto overscroll-contain p-0.5 sm:grid-cols-2">
            <button type="button" onClick={() => chooseSubcategory(activeCategoryId, 'all')} className="flex min-w-0 items-center justify-between gap-3 rounded-xl border border-[#d4a84f]/70 bg-[#d4a84f]/15 px-3 py-2.5 text-start text-xs font-black text-[#f8df9b] transition hover:bg-[#d4a84f]/25">
              <span className="min-w-0 break-words leading-relaxed">{isAr ? activeCategory?.allAr || 'الكل' : activeCategory?.allEn || 'All'}</span><span className="shrink-0 text-[#edc56d]">✓</span>
            </button>
            {getSubcategoriesForCategory(activeCategoryId).map(sub => (
              <button key={sub.id} type="button" onClick={() => chooseSubcategory(activeCategoryId, sub.id)} className="flex min-w-0 items-center justify-between gap-3 rounded-xl border border-[#f4d78d]/25 bg-[#4a0913]/75 px-3 py-2.5 text-start text-xs font-semibold text-[#fff0c8] transition hover:border-[#f4d78d]/70 hover:bg-[#791524]">
