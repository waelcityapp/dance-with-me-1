...
export const CreateEventPage: React.FC<CreateEventPageProps> = ({ onComplete, onCancel, initialAdType = null }) => {
  ...
  const [organizerName, setOrganizerName] = useState(
    savedDraft?.organizerName ?? (editingEvent && editingEvent.contact?.organizerName ? editingEvent.contact.organizerName : (user?.name || ''))
  );
  ...
  const clearSavedDraft = () => {
    try { localStorage.removeItem(CREATE_AD_DRAFT_KEY); } catch (e) {}
    ...
    <div className="w-full max-w-3xl mx-auto pt-2 pb-36 sm:pb-44" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {hasRestoredDraft && !editingEvent && (
        <div className="mb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-2xl border border-emerald-400/40 bg-emerald-50 p-4 text-sm text-emerald-950 dark:bg-emerald-500/10 dark:text-emerald-100">
          <div><p className="font-bold">{lang === 'ar' ? 'تمت استعادة مسودة إعلانك.' : 'Your ad draft was restored.'}</p><p className="mt-1 text-xs opacity-80">{lang === 'ar' ? 'يمكنك إكمال البيانات من حيث توقفت.' : 'You can continue from where you stopped.'}</p></div>
          <button type="button" onClick={clearSavedDraft} className="rounded-xl border border-emerald-600/30 px-3 py-2 text-xs font-bold hover:bg-emerald-100 dark:hover:bg-emerald-500/20">{lang === 'ar' ? 'إلغاء المسودة والبدء من جديد' : 'Discard draft and start over'}</button>
        </div>
      )}
                ...
                {contentLangMode === 'both' ? (
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      className="placeholder-neutral-400 dark:placeholder-neutral-600 w-full rounded-xl border border-neutral-200 bg-white text-neutral-900 focus:border-[#78101F] dark:border-neutral-800 dark:bg-neutral-950 dark:text-white dark:focus:border-amber-500 py-3 px-3 text-xs sm:text-sm font-mono outline-none"
                    />
                    <input
                      type="text"
                      value={priceEn}
                  ...
                  <input
                    ...
                    onChange={e => setPriceAr(e.target.value)}
                    placeholder={lang === 'ar' ? 'اختياري — اتركه فارغًا إذا لا يوجد سعر' : 'Optional — leave blank if there is no price'}
                    className="placeholder-neutral-400 dark:placeholder-neutral-600 w-full rounded-xl border border-neutral-200 bg-white text-neutral-900 focus:border-[#78101F] dark:border-neutral-800 dark:bg-neutral-950 dark:text-white dark:focus:border-amber-500 py-3 px-4 text-xs sm:text-sm font-mono outline-none"
                  />
                ) : (
                  <input
                    type="text"
