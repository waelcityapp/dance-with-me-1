...
// There are two motion.div's in the file, one was the wrapper, the other was for the accordion.
// Let's replace </motion.div> with </div> for the wrapper, but NOT for the accordion.

// Wait, I already removed <motion.div> for the main wrapper and replaced it with <div...
// Let's just restore the original ActualAttendanceModal completely and clean up.

const originalCode = `import React, { useState } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { X, Users, CheckCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
export const ActualAttendanceModal: React.FC<ActualAttendanceModalProps> = ({ eventId, onClose }) => {
  const { lang, bookings, events } = useApp();
  ...
  // Filter bookings for this event that are approved
  const eventBookings = bookings.filter(b => b.eventId === eventId && b.status === 'approved');
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
      <div
        className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 w-full max-w-md shadow-2xl relative max-h-[85vh] flex flex-col"
      >
        <button
          onClick={onClose}
...
fs.writeFileSync('src/components/modals/ActualAttendanceModal.tsx', originalCode);
