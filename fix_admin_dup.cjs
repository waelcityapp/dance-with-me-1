...
const firstIndex = content.indexOf(target1);
if (firstIndex !== -1) {
  content = content.substring(0, firstIndex) + content.substring(firstIndex + target1.length);
}

fs.writeFileSync('src/components/events/EventCard.tsx', content);
console.log('done fixing dup admin toolbar');
